...
install_plugin_from_git () {
  ...
  # Run composer install if composer.json exists and vendor directory is missing
  if [ -f "$plugin_path/composer.json" ]; then
    if [ ! -d "$plugin_path/vendor" ]; then
      echo "📦 Rodando composer install para $package_id..."
      composer install --working-dir="$plugin_path" --no-dev --optimize-autoloader --no-interaction
      chown -R www-data:www-data "$plugin_path/vendor"
    else
      echo "ℹ️ Vendor já existe para $package_id, pulando composer install."
    fi
  fi
...
install_plugin_if_not_exists woocommerce
...
install_plugin_from_git https://github.com/wp-graphql/wp-graphql-woocommerce wp-graphql-woocommerce
